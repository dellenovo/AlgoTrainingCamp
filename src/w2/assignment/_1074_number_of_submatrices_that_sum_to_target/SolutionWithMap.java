...
public class SolutionWithMap {
    ...
    void isQualified(int[][] matrix, int target, int x1, int x2, int y1, int y2) {
        int sum = sumMatrixWithMap(matrix, x1, x2, y1, y2);
    ...
    int sumMatrixWithMap(int[][] matrix, int x1, int x2, int y1, int y2) {
        int sum = 0;

        int upper = area.getOrDefault(calcMatrixKey(x1, x2, y1, y2 - 1), 0);
        int left = area.getOrDefault(calcMatrixKey(x1, x2 - 1, y1, y2), 0);
        int leftUpper = area.getOrDefault(calcMatrixKey(x1, x2 - 1, y1, y2 - 1), 0);

        sum = upper + left - leftUpper + matrix[y2][x2];
        area.put(calcMatrixKey(x1, x2, y1, y2), sum);
        return sum;
    }

    String calcMatrixKey(int x1, int x2, int y1, int y2) {
